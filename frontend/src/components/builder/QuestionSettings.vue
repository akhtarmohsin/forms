<template>
  <div class="p-4">
    ...
    <!-- Help text -->
    <div class="mb-4">
      <label class="block text-xs font-medium text-gray-500 mb-1">Description / Help Text</label>
      <textarea
    ...
    <div v-if="hasOptions" class="mb-4">
      ...
      <div class="space-y-1.5">
        ...
        >
          ...
          />
          <button @click="removeOption(i)" class="text-gray-300 hover:text-red-400 transition-colors flex-shrink-0">
    ...
    <div v-if="isTextType" class="mb-4 grid grid-cols-2 gap-3">
      <div>
        ...
        <input
          ...
          @input="$emit('update', { min_length: $event.target.value ? Number($event.target.value) : null })"
          class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="–"
        />
      </div>
      <div>
        ...
        <input
          ...
          @input="$emit('update', { max_length: $event.target.value ? Number($event.target.value) : null })"
          class="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
          placeholder="–"
        />
      </div>
    ...
    <label class="flex items-center gap-3 cursor-pointer mb-3">
      <div class="relative">
        <input
          ...
          class="sr-only peer"
        />
        <div class="w-9 h-5 bg-gray-200 rounded-full peer peer-checked:bg-blue-600 transition-colors" />
    ...
    <div class="border-t border-gray-100 pt-4 mt-4">
      ...
      <p class="text-xs text-gray-400">Advanced conditional logic can be configured after saving.</p>
    </div>
  </div>
</template>
...
watch(() => props.question?.options, (v) => {
...
const isTextType = computed(() => {
  ...
  return ["Short Answer", "Long Answer", "Paragraph", "Number", "Email", "Phone", "URL"].includes(t);
});
