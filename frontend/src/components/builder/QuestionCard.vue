<template>
	<div class="question-card cursor-pointer" :class="{ active }">
		<!-- Question header -->
		<div class="flex items-start gap-3">
			...
			<div class="flex-1 min-w-0">
				...
				<div class="flex items-start gap-2 mb-3">
					...
					<span
						v-if="q.required"
						class="text-red-500 text-sm font-semibold flex-shrink-0"
						>*</span
					>
				</div>
...
const previewComponent = computed(() => {
	...
	if (
		["Multiple Choice", "Checkbox", "Dropdown", "Yes/No", "True/False", "Ranking"].includes(
			type
		)
	)
	...
	if (["Date", "Time", "Date & Time"].includes(type)) return QuestionPreviewDate;
	if (["File Upload", "Signature"].includes(type)) return QuestionPreviewFile;
