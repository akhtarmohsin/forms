	...
	<div class="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-6">
		...
		<div class="grid grid-cols-2 gap-4">
			<div>
				...
				<input
					type="date"
					v-model="store.survey.start_date"
		...
		<div class="space-y-3">
			<label v-for="s in settings" :key="s.key" class="flex items-center justify-between">
				...
				<div class="relative flex-shrink-0 ml-4">
					<input
						type="checkbox"
						:checked="store.survey[s.key]"
						@change="store.updateSurveyMeta({ [s.key]: $event.target.checked })"
						class="sr-only peer"
					/>
					<div
						class="w-9 h-5 bg-gray-200 rounded-full peer peer-checked:bg-blue-600 transition-colors cursor-pointer"
					/>
					<div
						class="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform peer-checked:translate-x-4 pointer-events-none"
					/>
				</div>
			</label>
		</div>
...
const settings = [
	{
		key: "allow_anonymous_responses",
		label: "Allow anonymous responses",
		desc: "Responses won't be linked to user accounts",
	},
	{
		key: "allow_multiple_responses",
		label: "Allow multiple responses",
		desc: "Same user can submit more than once",
	},
	{ key: "require_login", label: "Require login", desc: "Only logged-in users can respond" },
	{
		key: "display_progress_bar",
		label: "Show progress bar",
		desc: "Display progress indicator at the top",
	},
	{
		key: "shuffle_questions",
		label: "Shuffle questions",
		desc: "Randomize question order for each respondent",
	},
];
</script>
