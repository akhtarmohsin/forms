<template>
	<div :style="cardStyle" class="p-6 shadow-sm border border-gray-100">
		...
		<p v-if="question.description" class="text-sm text-gray-400 mb-4">
		...
		<div
			v-else-if="
				['Multiple Choice', 'Yes/No', 'True/False'].includes(question.question_type)
			"
			class="space-y-2"
		>
			<label
			...
			>
				<div
					...
					:style="
						answer === opt
							? { borderColor: theme.primary_color, background: theme.primary_color }
							: { borderColor: '#d1d5db' }
					"
				>
		...
		<div v-else-if="question.question_type === 'Checkbox'" class="space-y-2">
			...
			>
				<div
					...
					:style="
						checkboxSelected(opt)
							? { borderColor: theme.primary_color, background: theme.primary_color }
							: { borderColor: '#d1d5db' }
					"
				>
					<svg
						v-if="checkboxSelected(opt)"
		...
		<div v-else-if="question.question_type === 'File Upload'">
			<label
				class="border-2 border-dashed border-gray-200 rounded-xl p-6 flex flex-col items-center cursor-pointer hover:border-blue-300 hover:bg-blue-50 transition-colors"
			>
				<svg
...
function checkboxSelected(opt) {
	...
	const vals = Array.isArray(props.answer)
		? props.answer
		: String(props.answer)
				.split(",")
				.map((s) => s.trim());
