    ...
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[80vh] flex flex-col overflow-hidden">
      ...
      <div class="overflow-y-auto flex-1 p-6">
        ...
        <div v-if="loading" class="grid grid-cols-2 gap-3">
          <div v-for="i in 6" :key="i" class="h-24 bg-gray-100 rounded-xl animate-pulse" />
        </div>
        <div v-else>
          <p class="text-xs font-medium text-gray-400 uppercase tracking-wider mb-3">Templates</p>
          <div class="grid grid-cols-2 gap-3">
            ...
            >
              <div class="w-9 h-9 rounded-lg flex-shrink-0 flex items-center justify-center text-white text-base"
                :style="{ background: categoryColor(t.category) }">
                {{ categoryEmoji(t.category) }}
              </div>
              <div class="min-w-0">
                <div class="font-medium text-gray-900 text-sm truncate">{{ t.template_name }}</div>
...
function categoryColor(c) { return COLORS[c] || "#6b7280"; }
function categoryEmoji(c) { return EMOJIS[c] || "📋"; }
</script>
