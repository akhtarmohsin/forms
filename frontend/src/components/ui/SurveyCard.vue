<template>
  <div class="bg-white rounded-xl border border-gray-200 p-5 hover:shadow-md transition-shadow group relative flex flex-col">
    <!-- Status badge -->
      ...
      <div class="relative">
        ...
        <transition name="fade">
          ...
          >
            ...
            <button @click="act('view-responses')" class="menu-item">View Responses</button>
            ...
            <button @click="act('delete')" class="menu-item text-red-600 hover:bg-red-50">Delete</button>
          </div>
        </transition>
      </div>
    </div>
    ...
    >{{ survey.survey_title || "Untitled Form" }}</h3>

    <!-- Meta -->
    <div class="text-xs text-gray-400 mt-auto pt-3 border-t border-gray-100 flex items-center justify-between">
      <span>{{ survey.total_responses || 0 }} response{{ survey.total_responses !== 1 ? "s" : "" }}</span>
...
const emit = defineEmits(["edit", "delete", "duplicate", "view-responses", "analytics", "toggle-status"]);
