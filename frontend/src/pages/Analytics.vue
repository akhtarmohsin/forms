			...
			<div v-else-if="activeTab === 'Summary'">
				...
				<div class="bg-white rounded-xl border border-gray-200 p-6 mb-6">
					...
					<div class="h-48 flex items-end gap-1.5">
						...
						>
							<div
								class="absolute -top-8 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-xs px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap"
							>
								{{ d.day }}: {{ d.count }}
							</div>
						</div>
					...
					<div
						v-if="!analytics?.daily_responses?.length"
			...
			<div v-else-if="activeTab === 'Questions'">
				...
				>
					...
					<p class="text-xs text-gray-400 mb-4">
						{{ q.question_type }} · {{ q.total_answers }} responses
					</p>
					...
					<div v-if="q.distribution" class="space-y-2">
						...
						>
							<span class="text-sm text-gray-600 w-32 truncate flex-shrink-0">{{
								option
							}}</span>
							<div class="flex-1 bg-gray-100 rounded-full h-3 overflow-hidden">
								<div
									...
									:style="{
										width: `${((count / q.total_answers) * 100).toFixed(1)}%`,
									}"
								/>
							</div>
				...
				<div
					v-if="!analytics?.question_stats?.length"
				...
				<div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
					...
					>
						<span class="text-sm font-medium text-gray-700"
							>{{ responses.length }} response(s)</span
						>
						<button
						...
						<tbody class="divide-y divide-gray-100">
							...
							>
								<td class="px-6 py-4 text-gray-900">
									{{ r.respondent_name || r.email || "Anonymous" }}
								</td>
								<td class="px-6 py-4">
									<span
